...
void validate_reqstring(char *reqstring) {

    int i, reqstring_length, x=0;
    reqstring_length = strlen(reqstring);
    char new_reqstring[reqstring_length];

    while (i<reqstring_length) {
        if (reqstring[i] == '.' && reqstring[i+1] == '.') {
            i++;
        }
        else {
            new_reqstring[x] = reqstring[i];
            i++;
            x++;
        }
    }

    for (i=0; i<strlen(new_reqstring); i++) {
        printf("new: %c\n", new_reqstring[i]);
    }
